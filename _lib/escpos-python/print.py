#!/usr/bin/env -S sh -c '"`dirname $0`/venv_python/bin/python3" "$0" "$@"'
# https://stackoverflow.com/questions/20095351/shebang-use-interpreter-relative-to-the-script-path

#################################################
# ATTENTION: NOTICE: Alternative #!/usr/bin/env necessary for virtual environments.

#/usr/bin/env -S sh -c '"`dirname $0`/bin/python3" "$0" "$@"'
# https://stackoverflow.com/questions/20095351/shebang-use-interpreter-relative-to-the-script-path
...
def _interact_escpos(current_devfile = '/dev/ttyS1', current_baud = 38400):
	p = Serial(devfile=current_devfile,
...
def _qr(current_content = '', current_devfile = '/dev/ttyS1', current_baud = 38400, current_text = ''):
	p = Serial(devfile=current_devfile,
		baudrate=current_baud,
		bytesize=8,
		parity='N',
	...
	p.qr(current_content, QR_ECLEVEL_H)
...
def _barcode(current_content = '', current_devfile = '/dev/ttyS1', current_baud = 38400):
	p = Serial(devfile=current_devfile,
		baudrate=current_baud,
		bytesize=8,
		parity='N',
	...
	#   Apparently valid.
	# code39
	# NOTICE: Disable UCC/EAN128 on your barcode scanner to enable code128 arbitrary text.
	#barcode(code, bc, height=64, width=3, pos='BELOW', font='A', align_ct=True, function_type=None, check=True, force_software=False)
	#p.barcode(current_content, "EAN13")
	p.barcode(current_content, "code128", 64, 3, 'BELOW', 'B', False, None, True, True)
...
def _seed(current_content = '', current_devfile = '/dev/ttyS1', current_baud = 38400, current_text = ""):
	p = Serial(devfile=current_devfile,
		baudrate=current_baud,
		bytesize=8,
		parity='N',
