		...
		then
			case "$current_bitmask" in
				0)
					...
					;;
				1)
					_extractEntropyAlpha_bin _extractEntropyBin 10000000000 | base64 | "$current_filter" | tr -dc '0-9' | head -c 1
					;;
				2)
				...
				8)
					...
					;;
				3)
					_extractEntropyAlpha_bin _extractEntropyBin 10000000000 | base64 | "$current_filter" | tr -dc 'a-z0-9' | head -c 1
					;;
				5)
					_extractEntropyAlpha_bin _extractEntropyBin 10000000000 | base64 | "$current_filter" | tr -dc 'A-Z0-9' | head -c 1
					;;
				7)
					_extractEntropyAlpha_bin _extractEntropyBin 10000000000 | base64 | "$current_filter" | tr -dc 'a-zA-Z0-9' | head -c 1
					;;
			esac
		fi
	done
